...
Red doesn’t currently have built-in list comprehensions. Here is a simple function that takes a block as specification and returns a new list generated according the rules in the specification:
...
gen-list: func [
    ...
    either init [
        ...
        either all [list-map/:id list-map/:id/idx < list-map/:id/limit][
            until [
                list-map/:id/idx: list-map/:id/idx + 1
                values: mixed-base list-map/:id/idx - 1 list-map/:id/bases
                repeat idx length? values [
                    values/:idx: list-map/:id/lists/:idx/(values/:idx)
                ]
                do reduce append copy [:list-map/:id/code] values
            ]    
        ][
            none
        ]
...
The first call to `gen-list` must be with the `/init` refinement – it will create a record in the functions’s internal map for the `is` that is provided. The next calls should be just `gen-list 'id`:
