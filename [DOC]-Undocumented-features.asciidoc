...
`#include-binary` preprocessor directive accepts `file!` or `string!` argument that denotes a file path to binary data to be inlined as `binary!` value. Useful for embedding application resources, at the cost of beefing up the resulting executable size.